...
import {serveStatic} from "restify";
...
export default class Server {
    ...
    static insight: InsightFacade;

    constructor(port: number) {
        Log.info("Server::<init>( " + port + " )");
        this.port = port;
        if (Server.insight === undefined) {
    ...
    public start(): Promise<boolean> {
        ...
        return new Promise(function (fulfill, reject) {
            try {
                ...
                //Modified GET
                let courseFile = new Buffer(readFileSync("courses.zip")).toString("Base64");
                let roomFile = new Buffer(readFileSync("rooms.zip")).toString("Base64");
                Server.insight.addDataset("courses", courseFile);
                Server.insight.addDataset("rooms", roomFile);
    ...
    public static put(req: restify.Request,res: restify.Response,next: restify.Next) {
        ...
        try {
            ...
            Server.insight.addDataset(id, dataStr).then(function (response: InsightResponse) {
                res.json(response.code, response.body);
                return next();
            }) .catch(function (errRes) {
                res.json(errRes.code, errRes.body);
                return next();
            })
        }
        catch (err) {
    ...
    public static del(req: restify.Request,res: restify.Response,next: restify.Next) {
        try {
            let id = req.params.id;
            Server.insight.removeDataset(id).then(function (response: InsightResponse) {
                res.json(response.code, response.body);
                return next();
            }) .catch(function (errRes) {
                res.json(errRes.code, errRes.body);
    ...
    public static post(req: restify.Request,res: restify.Response,next: restify.Next) {
        try {
            let query = req.body;
            Server.insight.performQuery(query).then(function (response: InsightResponse) {
                res.json(response.code, response.body);
                return next();
            }).catch(function (errRes) {
                res.json(errRes.code, errRes.body);
